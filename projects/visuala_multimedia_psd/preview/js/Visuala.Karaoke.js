...
//	t.audio.addEventListener('timeupdate', function() { t.selectWord(); });
	...
	t.audio.addEventListener('play', this.startEvent, false);
	t.audio.addEventListener('ended', this.stopEvent, false);
	t.audio.addEventListener('seeked', this.selectEvent, false);
...
AudioAligner.prototype = {
	...
	close: function() {
		var t = this;
		t.audio.removeEventListener('play', this.startEvent, false);
		t.audio.removeEventListener('ended', this.stopEvent, false);
		t.audio.removeEventListener('seeked', this.selectEvent, false);
	},
...
(function ( $ ) {
    ...
    $.fn.visualaKaraoke = function( options, arg ) { 
		...
		if(typeof options === 'object')
	        {
			// normally we should create jQuery plugin only once, but with Blockly, for simplicity, we recreate over and over again(because its only one command / block that way), so we make sure to deinit any old plugin instance on this element
			if (this.data('visualaKaraoke') != null) {
				this.data('visualaKaraoke').close();
			}                                   
//			if (this.data('visualaKaraoke') == null) {
				// make sure we create one unique audio player per karaoke target element
				// creating only one audio player for all karaoke would be an isuse because of event handlers
				// from other karaokes still lurking around and triggering.

				if ($('#visualaKaraokeAudio_' + this.attr('id')).length == 0) {
					$("body").append('<audio id="visualaKaraokeAudio_' + this.attr('id') + '"></audio>');
				}
				settings = $.extend({
					...
					json: null,	
					play: true,	
					elementPlayer: $('#visualaKaraokeAudio_' + this.attr('id'))[0],		
					onComplete: function() {}
				}, options );
	
				// init plugin
				this.data("visualaKaraokeSettings", settings);
...
/*			} else {
				if (this.data('visualaKaraokeSettings').play) this.data('visualaKaraoke').align();
			}*/
